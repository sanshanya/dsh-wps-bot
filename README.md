...
2. ✅ **真实联通自证**（2026-08-16 全绿，证据+事故登记见 docs/acceptance-live.md）：真 WPS 租户 + 真 LLM profile 合跑文本问答、进度卡片、限时窗三条主场景全部通过。残留 U1/U2 两项发版闸见同文档。
   - **首帧抓包**：✅ ① @bot 群消息真命中在 `mentions[].identity.id == spId`（`id` 是下标串，另有 `<at id>N 展示名</at>` 兜底）；② bot 自发 `sender.type=="sp"` 且卡片自发 `content==null`——均固化进 `test/protocol.test.ts` 的 `REAL_FRAMES`；③ **WS 群聊只推 @bot**（时间戳级存证见 `docs/acceptance-live.md` 附录）：附件触达唯一入路=发消息时顺带 @bot；群聊附件必须 @ 它才触达，私聊直发即可。
